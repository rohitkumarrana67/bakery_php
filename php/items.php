<?php
	...
	if($_GET['product'] == 'cakes'){
    	$sql = "create table IF NOT EXISTS " . $_GET['product'] . "(id INT AUTO_INCREMENT, title VARCHAR(255) NOT NULL, cost INT, rating FLOAT, summary VARCHAR(255), reviews VARCHAR(255), img VARCHAR(255),primary key (id))";
		...
		$sql = 'SELECT COUNT(*) FROM ' . $_GET['product'];
    	$result = mysqli_query($conn,$sql);
    	$data=mysqli_fetch_assoc($result);

		if($data['COUNT(*)'] == 0){
			foreach ($cakes as $cake) {
				$sql = "INSERT INTO " . $_GET['product'] ." (title,cost,rating,summary,reviews,img) VALUES('$cake[0]','$cake[1]','$cake[2]','$cake[3]','$cake[4]','$cake[5]')";
    	...
    	if($data['COUNT(*)'] == 0){
    		foreach ($cupcakes as $item) {
				$sql = "INSERT INTO " . $_GET['product'] ." (title,cost,rating,summary,reviews,img) VALUES('$item[0]','$item[1]','$item[2]','$item[3]','$item[4]','$item[5]')";
